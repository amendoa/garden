...
class LazyLoad {
  ...
  checkVisiblePlaceholders() {
    this.windowHeight = typeof window != "undefined" ? $(window).height() : 0;
    this.windowWidth = typeof window != "undefined" ? $(window).width() : 0;

    Array.prototype.forEach.call(
      this.$element,
      this.checkPlaceholder.bind(this)
    );
  }
  ...
  isPlaceholderVisible(placeholder) {
    return (
      ...
      this.windowHeight + this.options.offset
    );
  }

  renderImage(placeholder) {
    placeholder.parentNode.replaceChild(
      this.createImage(placeholder),
      placeholder
    );
  }
  ...
  parseAttributes(image, attributes) {
    Array.prototype.forEach.call(attributes, attr => {
      if (
        attr.name !== this.options.selector ||
